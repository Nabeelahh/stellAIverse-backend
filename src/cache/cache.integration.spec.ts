...
describe('Cache Integration Tests', () => {
  ...
  describe('Dependency-Aware Invalidation', () => {
    it('should invalidate dependent caches when upstream job completes', async () => {
      ...
      let result1 = await cacheService.get(downstreamJobType1, downstreamPayload1);
      let result2 = await cacheService.get(downstreamJobType2, downstreamPayload2);
    ...
    it('should handle cascading invalidation in DAG', async () => {
      ...
      await cacheService.set('processing', { source: job1 }, { result: 'job2' }, job2, undefined, config1);

      // Set up cache for job3 (depends on job2)
      await cacheService.set('reporting', { source: job2 }, { result: 'job3' }, job3, undefined, config2);

      // Verify caches exist
      let result2 = await cacheService.get('processing', { source: job1 });
      let result3 = await cacheService.get('reporting', { source: job2 });
      expect(result2).not.toBeNull();
      expect(result3).not.toBeNull();

      // Invalidate job1, should cascade to job2 and job3
      let invalidatedCount = await cacheService.invalidateDependents(job1);
  ...
  describe('Concurrent Access', () => {
    it('should handle concurrent cache operations', async () => {
      ...
      for (let i = 0; i < jobCount; i++) {
        readOperations.push(
          cacheService.get('data-processing', { id: i }),
        );
      }
  ...
  describe('Cache Warming', () => {
    ...
    it('should track cache warming status', async () => {
      ...
      const jobs = [
        { jobType: 'data-processing', payload: { id: 1 } },
      ];
  ...
  describe('Cache Job Plugin', () => {
    it('should check cache and return result if hit', async () => {
      ...
      // Pre-populate cache
      const config = new CacheConfigDto();
      await cacheService.set(jobType, payload, result, 'job-1', undefined, config);
    ...
    it('should store result in cache after job completion', async () => {
      ...
      const storageResult = await cacheJobPlugin.storeResult(mockJob as any, result);
  ...
  describe('Metrics and Monitoring', () => {
    it('should aggregate cache metrics', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const config = new CacheConfigDto();
        await cacheService.set('data-processing', { id: i }, { result: i }, undefined, undefined, config);
      }
  ...
  describe('Version-Based Invalidation', () => {
    it('should invalidate cache when job definition changes', async () => {
      ...
      const invalidatedCount =
        await cacheService.validateAndInvalidateOldVersions(jobType, newVersion);
