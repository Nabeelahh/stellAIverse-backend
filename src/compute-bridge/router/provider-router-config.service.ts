...
export class ProviderRouterConfigService {
  ...
  private getHealthCheckInterval(): number {
    return this.configService.get<number>('COMPUTE_HEALTH_CHECK_INTERVAL', 30000);
  }
  ...
  private getCircuitBreakerConfig(): CircuitBreakerConfig {
    return {
      failureThreshold: this.configService.get<number>('COMPUTE_CIRCUIT_BREAKER_FAILURE_THRESHOLD', 5),
      recoveryTimeout: this.configService.get<number>('COMPUTE_CIRCUIT_BREAKER_RECOVERY_TIMEOUT', 30000),
      successThreshold: this.configService.get<number>('COMPUTE_CIRCUIT_BREAKER_SUCCESS_THRESHOLD', 3),
      backoffMultiplier: this.configService.get<number>('COMPUTE_CIRCUIT_BREAKER_BACKOFF_MULTIPLIER', 2),
      maxBackoffTime: this.configService.get<number>('COMPUTE_CIRCUIT_BREAKER_MAX_BACKOFF', 300000)
    };
  }
  ...
  private getProviderWeights(): ProviderWeight[] {
    ...
    const openaiWeight = this.configService.get<number>('COMPUTE_PROVIDER_OPENAI_WEIGHT');
    if (openaiWeight) {
      weights.push({
        provider: AIProviderType.OPENAI,
        weight: openaiWeight,
        costFactor: this.configService.get<number>('COMPUTE_PROVIDER_OPENAI_COST_FACTOR', 1),
        latencyFactor: this.configService.get<number>('COMPUTE_PROVIDER_OPENAI_LATENCY_FACTOR', 1)
      });
    }
    
    // Anthropic configuration
    const anthropicWeight = this.configService.get<number>('COMPUTE_PROVIDER_ANTHROPIC_WEIGHT');
    if (anthropicWeight) {
      weights.push({
        provider: AIProviderType.ANTHROPIC,
        weight: anthropicWeight,
        costFactor: this.configService.get<number>('COMPUTE_PROVIDER_ANTHROPIC_COST_FACTOR', 1.2),
        latencyFactor: this.configService.get<number>('COMPUTE_PROVIDER_ANTHROPIC_LATENCY_FACTOR', 0.8)
      });
    }
    
    // Google configuration
    const googleWeight = this.configService.get<number>('COMPUTE_PROVIDER_GOOGLE_WEIGHT');
    if (googleWeight) {
      weights.push({
        provider: AIProviderType.GOOGLE,
        weight: googleWeight,
        costFactor: this.configService.get<number>('COMPUTE_PROVIDER_GOOGLE_COST_FACTOR', 0.8),
        latencyFactor: this.configService.get<number>('COMPUTE_PROVIDER_GOOGLE_LATENCY_FACTOR', 1.1)
      });
    }
    
    // HuggingFace configuration
    const huggingfaceWeight = this.configService.get<number>('COMPUTE_PROVIDER_HUGGINGFACE_WEIGHT');
    if (huggingfaceWeight) {
      weights.push({
        provider: AIProviderType.HUGGINGFACE,
        weight: huggingfaceWeight,
        costFactor: this.configService.get<number>('COMPUTE_PROVIDER_HUGGINGFACE_COST_FACTOR', 0.5),
        latencyFactor: this.configService.get<number>('COMPUTE_PROVIDER_HUGGINGFACE_LATENCY_FACTOR', 1.5)
      });
    }
  ...
  private getDefaultFallbackChain(): AIProviderType[] {
    const fallbackChain = this.configService.get<string>('COMPUTE_FALLBACK_CHAIN');
    if (fallbackChain) {
      return fallbackChain.split(',').map(provider => provider.trim() as AIProviderType);
    }
  ...
  private getMaxConcurrentRequests(): Map<AIProviderType, number> {
    ...
    const defaultMax = this.configService.get<number>('COMPUTE_MAX_CONCURRENT_REQUESTS', 100);
    
    maxRequests.set(AIProviderType.OPENAI, 
      this.configService.get<number>('COMPUTE_MAX_CONCURRENT_OPENAI', defaultMax));
    maxRequests.set(AIProviderType.ANTHROPIC, 
      this.configService.get<number>('COMPUTE_MAX_CONCURRENT_ANTHROPIC', defaultMax));
    maxRequests.set(AIProviderType.GOOGLE, 
      this.configService.get<number>('COMPUTE_MAX_CONCURRENT_GOOGLE', defaultMax));
    maxRequests.set(AIProviderType.HUGGINGFACE, 
      this.configService.get<number>('COMPUTE_MAX_CONCURRENT_HUGGINGFACE', defaultMax));
