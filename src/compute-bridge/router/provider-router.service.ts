...
import { CircuitBreakerService, CircuitBreakerEvent } from './circuit-breaker.service';
...
export class ProviderRouterService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.circuitBreakerService.addEventListener(this.handleCircuitBreakerEvent.bind(this));
  ...
  registerProvider(provider: IAIProvider): void {
    ...
    this.circuitBreakerService.initializeCircuitBreaker(providerType, this.config.circuitBreaker);
    ...
    try {
      ...
      switch (request.context.strategy || this.config.defaultStrategy) {
        case LoadBalancingStrategy.ROUND_ROBIN:
          ({ provider: selectedProvider, reason } = this.selectRoundRobin(candidates));
          break;
          
        case LoadBalancingStrategy.WEIGHTED:
          ({ provider: selectedProvider, reason } = this.selectWeighted(candidates, request.context));
          break;
          
        case LoadBalancingStrategy.LEAST_CONNECTIONS:
          ({ provider: selectedProvider, reason } = this.selectLeastConnections(candidates));
          break;
          
        case LoadBalancingStrategy.RANDOM:
          ({ provider: selectedProvider, reason } = this.selectRandom(candidates));
          break;
          
        case LoadBalancingStrategy.HEALTH_AWARE:
          ({ provider: selectedProvider, reason } = this.selectHealthAware(candidates));
          break;
          
        case LoadBalancingStrategy.COST_OPTIMIZED:
          ({ provider: selectedProvider, reason } = this.selectCostOptimized(candidates, request.context));
          break;
      ...
      const healthMetrics = this.healthService.getHealthMetrics(selectedProvider);
      const expectedResponseTime = healthMetrics?.responseTime || 0;
      const expectedCost = this.calculateExpectedCost(selectedProvider, request);
      ...
      const result: SelectedProvider = {
        ...
        expectedCost,
        routingPath: [`strategy:${request.context.strategy || this.config.defaultStrategy}`],
        fallbackHistory: []
      };

      this.logger.debug(`Provider selected: ${selectedProvider} - ${reason} (${Date.now() - startTime}ms)`);
      return result;
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (!this.circuitBreakerService.canExecute(selectedProvider.provider)) {
          throw new Error(`Circuit breaker open for provider: ${selectedProvider.provider}`);
        }
        
        // Execute request
        const startTime = Date.now();
        const result = await executor(selectedProvider.provider, request.request);
        ...
        if (selectedProvider) {
          ...
          this.circuitBreakerService.recordFailure(selectedProvider.provider, error.message);
        ...
        this.logger.warn(`Request failed (attempt ${attempt + 1}/${maxRetries}): ${error.message}`);
  ...
  getAvailableProviders(): AIProviderType[] {
    return this.circuitBreakerService.getAvailableProviders()
  ...
  private getCandidateProviders(context: RoutingContext): AIProviderType[] {
    ...
    if (context.preferredProviders && context.preferredProviders.length > 0) {
      ...
      candidates = context.fallbackChain || 
   ...
   */
  private selectRoundRobin(candidates: AIProviderType[]): { provider: AIProviderType; reason: string } {
   ...
   * Weighted selection
   */
  private selectWeighted(candidates: AIProviderType[], context: RoutingContext): { provider: AIProviderType; reason: string } {
    // Simplified weighted selection
    const weights = candidates.map(provider => {
      const weightConfig = this.config.providerWeights.find(w => w.provider === provider);
   ...
   */
  private selectLeastConnections(candidates: AIProviderType[]): { provider: AIProviderType; reason: string } {
   ...
   * Random selection
   */
  private selectRandom(candidates: AIProviderType[]): { provider: AIProviderType; reason: string } {
   ...
   */
  private selectHealthAware(candidates: AIProviderType[]): { provider: AIProviderType; reason: string } {
    let bestProvider = candidates[0];
   ...
   */
  private selectCostOptimized(candidates: AIProviderType[], context: RoutingContext): { provider: AIProviderType; reason: string } {
    let bestProvider = candidates[0];
    let bestScore = Infinity;
  ...
  private calculateHealthScore(metrics: ProviderHealthMetrics): number {
    ...
    const healthScore = metrics.status === ProviderHealthStatus.HEALTHY ? 1 : 
                       metrics.status === ProviderHealthStatus.DEGRADED ? 0.5 : 0;
   ...
   */
  private calculateExpectedCost(provider: AIProviderType, request: ComputeRequest): number {
   ...
   */
  private recordRequestSuccess(provider: AIProviderType, responseTime: number): void {
    ...
    if (stats) {
      ...
      if (stats.averageResponseTime === 0) {
        ...
        stats.averageResponseTime = 0.9 * stats.averageResponseTime + 0.1 * responseTime;
      }
  ...
  private handleCircuitBreakerEvent(event: CircuitBreakerEvent): void {
    this.logger.warn(`Circuit breaker event: ${event.provider} -> ${event.state} (${event.reason})`);
