...
export class ProviderMetricsService {
  ...
  recordRequestStart(provider: string, requestType: string): void {
    this.requestsTotal.inc({ provider, request_type: requestType, status: 'started' });
  }

  /**
   * Record a successful request
   */
  recordRequestSuccess(provider: string, requestType: string, duration: number): void {
    this.requestsTotal.inc({ provider, request_type: requestType, status: 'success' });
    this.requestDuration.observe({ provider, request_type: requestType }, duration / 1000);
  }

  /**
   * Record a failed request
   */
  recordRequestError(provider: string, requestType: string, errorType: string): void {
    this.requestsTotal.inc({ provider, request_type: requestType, status: 'error' });
    this.requestErrors.inc({ provider, request_type: requestType, error_type: errorType });
  }

  /**
   * Update provider health metrics
   */
  updateProviderHealth(provider: string, healthStatus: string, responseTime: number, connections: number, successRate: number): void {
    const healthValue = healthStatus === 'healthy' ? 1 : 
                       healthStatus === 'degraded' ? 0.5 : 0;
  ...
  updateCircuitBreakerState(provider: string, state: string): void {
    const stateValue = state === 'closed' ? 1 : 
                      state === 'half_open' ? 0.5 : 0;
   ...
   */
  recordCircuitBreakerTransition(provider: string, fromState: string, toState: string): void {
   ...
   */
  recordRoutingDecision(provider: string, strategy: string, reason: string): void {
   ...
   */
  recordFallbackEvent(fromProvider: string, toProvider: string, reason: string): void {
    this.fallbackEvents.inc({ from_provider: fromProvider, to_provider: toProvider, reason });
  }
