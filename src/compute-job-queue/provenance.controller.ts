...
export class ProvenanceController {
  ...
  async getDependentJobs(@Param('id') jobId: string) {
    const dependentJobIds = await this.provenanceService.getDependentJobs(jobId);
    return {
      jobId,
  ...
  async rerunJob(@Param('id') originalJobId: string, @Body() rerunDto: JobRerunDto) {
    const originalProvenance = await this.provenanceService.getProvenanceByJobId(originalJobId);
    if (!originalProvenance) {
      throw new NotFoundException(`Original job ${originalJobId} not found`);
    }

    const canReproduce = await this.provenanceService.canReproduce(originalJobId);
    if (!canReproduce) {
      throw new BadRequestException('Job cannot be reproduced - missing dependencies');
    }
